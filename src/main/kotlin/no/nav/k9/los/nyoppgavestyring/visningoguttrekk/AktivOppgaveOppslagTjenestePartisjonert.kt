...
class AktivOppgaveOppslagTjenestePartisjonert(
    ...
    override fun hentAktivOppgave(eksternId: String, oppgavetype: String): Oppgave {
        return hentAktivOppgaveHvisFinnes(eksternId, oppgavetype)
            ?: throw IllegalStateException("Fant ikke aktiv oppgave med eksternId=$eksternId og oppgavetype=$oppgavetype")
    }

    override fun hentAktivOppgaveHvisFinnes(eksternId: String, oppgavetype: String): Oppgave? {
        return transactionalManager.transaction { tx ->
            ...
            val rad = tx.run(
                queryOf(
                    ...
                    """.trimIndent(),
                    mapOf("eksternId" to eksternId, "oppgavetype" to oppgavetype)
                ).map { row ->
                    OppgaveRad(
                        id = row.long("id"),
