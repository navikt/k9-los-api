...
import no.nav.k9.los.nyoppgavestyring.oppgaveuthenting.query.dto.felter.Verdiforklaring
import no.nav.k9.los.nyoppgavestyring.oppgaveuthenting.query.dto.resultat.OppgaveQueryRad
import no.nav.k9.los.nyoppgavestyring.oppgaveuthenting.query.mapping.OppgaveQueryToSqlMapper
import no.nav.k9.los.nyoppgavestyring.oppgaveuthenting.query.mapping.transientfeltutleder.GyldigeTransientFeltutleder
...
class OppgaveQueryRepository(
    ...
    fun query(
        ...
        loggSqlDebug(sqlBuilder)
        return tx.run(
            queryOf(sqlBuilder.getQuery(), sqlBuilder.getParams())
                .map(sqlBuilder::mapRowTilRad).asList
        )
    }
