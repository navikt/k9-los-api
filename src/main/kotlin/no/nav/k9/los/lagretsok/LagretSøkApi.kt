...
fun Route.LagretSøkApi() {
        ...
        medBrukerkontekst { bruker ->
            if (bruker.harBasisTilgang) {
                ...
                if (innloggetSaksbehandler == null) {
                    ...
                    val lagredeSøk = lagretSøkRepository.hentAlle(innloggetSaksbehandler, bruker.område)
        ...
        medBrukerkontekst { bruker ->
            if (bruker.harBasisTilgang) {
                ...
                if (innloggetSaksbehandler == null) {
                    ...
                    if (lagretSøk != null) {
                        if (lagretSøk.område != bruker.område || lagretSøk.lagetAv != innloggetSaksbehandler.id) {
                            call.respond(HttpStatusCode.Forbidden)
                        } else {
                            call.respond(lagretSøkTjeneste.hent(bruker, id))
                        }
                    } else {
    ...
    post("nytt", {
        ...
        medBrukerkontekst { bruker ->
            if (bruker.erOppgavestyrer) {
                ...
                val lagretSøk = lagretSøkTjeneste.nytt(bruker, request)
        ...
        medBrukerkontekst { bruker ->
            if (bruker.harBasisTilgang) {
                ...
                if (innloggetSaksbehandler == null) {
                    ...
                    if (call.parameters["id"]?.toLongOrNull() != endreLagretSøk.id) {
                        call.respond(HttpStatusCode.BadRequest, "Id i sti og body må være like")
                        return@medBrukerkontekst
                    }
                    val lagretSøk = lagretSøkTjeneste.endre(bruker, endreLagretSøk)
    ...
    post("{id}/kopier", {
        ...
        medBrukerkontekst { bruker ->
            if (bruker.harBasisTilgang) {
                val saksbehandler = bruker.navIdent.let {
                    saksbehandlerRepository.finnSaksbehandlerMedIdent(it, bruker.harTilgangTilKode6)
                }
                if (saksbehandler == null) {
                    ...
                    val nyttLagretSøk = lagretSøkTjeneste.kopier(bruker, lagretSøkId, tittel, saksbehandler)
    ...
    delete("{id}/slett", {
        ...
        medBrukerkontekst { bruker ->
            if (bruker.harBasisTilgang) {
                val lagretSøkId = call.parameters["id"]!!.toLong()
                lagretSøkTjeneste.slett(bruker, lagretSøkId)
        ...
        medBrukerkontekst { bruker ->
            if (bruker.harBasisTilgang) {
                ...
                if (innloggetSaksbehandler == null) {
                    ...
                    if (lagretSøk == null) {
                        call.respond(HttpStatusCode.NotFound)
                    } else if (lagretSøk.område != bruker.område || lagretSøk.lagetAv != innloggetSaksbehandler.id) {
                        call.respond(HttpStatusCode.Forbidden)
                    } else {
                        call.respond(lagretSøkTjeneste.hentAntall(bruker, lagretSøkId.toLong()))
                    }
                }
            } else {
                call.respond(HttpStatusCode.Forbidden)
