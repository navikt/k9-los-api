...
object BehandlingProsessEventK9TilbakeDuplikatUtil {

    private val log = LoggerFactory.getLogger(BehandlingProsessEventK9TilbakeDuplikatUtil::class.java)

    fun fjernDuplikater(eventer : Collection<BehandlingProsessEventTilbakeDto>) : Set<BehandlingProsessEventTilbakeDto> {
        val gruppert = eventer.groupBy { it.eventTid }
        val resultat : MutableSet<BehandlingProsessEventTilbakeDto> = mutableSetOf()
        ...
        for ((tidspunkt, eventerMedLikTid) in gruppert) {
            if (eventerMedLikTid.size == 1){
                ...
                val leggesTil: MutableSet<BehandlingProsessEventTilbakeDto> = mutableSetOf()
    ...
    fun erFunksjoneltLike(a : BehandlingProsessEventTilbakeDto, b: BehandlingProsessEventTilbakeDto) : Boolean{
        if (a == b) {
            return true
        }
    ...
    fun ulikeFelter(a : BehandlingProsessEventTilbakeDto, b: BehandlingProsessEventTilbakeDto) : List<String> {
