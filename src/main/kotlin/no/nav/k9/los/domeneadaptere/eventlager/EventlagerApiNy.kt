...
import no.nav.k9.los.forvaltning.krevFagsystem
...
import no.nav.k9.los.forvaltning.medDriftstilgang as medBrukerkontekstUtenOmråde
...
internal fun Route.EventlagerApiNy() {
    ...
    get("/eventer/{fagsystem}/{eksternId}", {
        ...
        medBrukerkontekstUtenOmråde { bruker ->
            if (bruker.harDriftstilgang) {
                ...
                forvaltningRepository.krevEventområde(fagsystem, bruker.område, eksternId)
    ...
    put("/spillAvDirtyEventer", {
        ...
        medBrukerkontekst { bruker ->
            bruker.krevK9Drift()
            Fagsystem.entries.filter { it != Fagsystem.UNGSAK && it != Fagsystem.UNGTILBAKE }
                .forEach { forvaltningRepository.krevEventområde(it, bruker.område) }
            thread(
                start = true,
                isDaemon = true,
    ...
    put("/{fagsystem}/bestillHistorikkvask", {
        ...
        medBrukerkontekst { bruker ->
            val fagsystem = Fagsystem.fraKode(call.parameters["fagsystem"]!!)
            bruker.krevFagsystem(fagsystem)
            forvaltningRepository.krevEventområde(fagsystem, bruker.område)
    ...
    put("bestillHistorikkvaskForEnkeltoppgave", {
        ...
        medBrukerkontekst { bruker ->
            val fagsystem = Fagsystem.fraKode(call.request.queryParameters["fagsystem"]!!)
            bruker.krevFagsystem(fagsystem)
