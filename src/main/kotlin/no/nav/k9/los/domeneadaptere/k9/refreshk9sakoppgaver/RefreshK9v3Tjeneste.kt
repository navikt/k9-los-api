package no.nav.k9.los.domeneadaptere.k9.refreshk9sakoppgaver
...
class RefreshK9v3Tjeneste(
    ...
    fun behandlingerTilOppfriskning(tx: TransactionalSession, antallPrKø: Int): Set<UUID> {
        return DetaljerMetrikker.time("RefreshK9V3", "refreshForKøer", "alle") {
            val alleKøer = oppgaveKoRepository.hentListe(område = Områder.K9, skjermet = false, medSaksbehandlere = false) +
                    oppgaveKoRepository.hentListe(område = Områder.K9, skjermet = true, medSaksbehandlere = false)
    ...
    private fun hentK9sakParsakOppgaver(oppgaver: Collection<Oppgave>): Set<EksternOppgaveId> {
        ...
        return oppgaveQueryService.queryForOppgaveEksternId(QueryRequest(Områder.K9, query)).toSet()
    }
