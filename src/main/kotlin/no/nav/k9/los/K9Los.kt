...
fun Application.k9Los() {
    ...
    if (LocalDateTime.now().isBefore(LocalDateTime.of(2025, 1, 9, 13, 0))) {
    ...
    install(Routing) {
        ...
        if ((KoinProfile.LOCAL == koin.get<KoinProfile>())) {
            ...
            localSetup.initK9SakOppgaver(1000)
            localSetup.initTilbakeoppgaver(1000)
            localSetup.initPunsjoppgaver(1000)
            api(sseChannel)
            route("/forvaltning") {
                InnsiktApis()
                forvaltningApis()
