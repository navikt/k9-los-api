...
class PepClient internal constructor(
    ...
    private suspend fun harTilgang(
        ...
        idToken: no.nav.k9.los.infrastruktur.idtoken.IdToken? = null,
    ): Boolean = when (område) {
        ...
        return when (oppgavetype) {
            "k9sak", "k9klage", "k9tilbake" -> {
                ...
                if (idToken != null) {
                    ...
                    klient.harTilgangTilSak(action, SaksnummerDto(saksnummer!!), identTilInnloggetBruker, saksbehandlersGrupper)
                }
            }
            ...
            "k9punsj" -> {
                ...
                val tilgang = if (aktørIder.isNotEmpty() && idToken != null) {
                    klient.harTilgangTilPersoner(action, aktørIder, idToken)
                } else if (aktørIder.isNotEmpty()) {
                    val saksbehandlersGrupper =
                        grupperForSaksbehandler ?: azureGraphService.hentGrupper(identTilInnloggetBruker)
                    klient.harTilgangTilPersoner(action, aktørIder, identTilInnloggetBruker, saksbehandlersGrupper)
                } else {
