...
class OppgaveKoTjeneste(
    ...
    fun hentAntallOppgaverForKø(
        ...
        { oppgaveQueryService.queryForAntall(QueryRequest(ko.oppgaveQuery, fjernReserverte = filtrerReserverte, område = ko.område, harTilgangTilKode6 = skjermet)) }
    }
    ...
    suspend fun hentSaksbehandlereForKo(oppgaveKoId: Long, brukerkontekst: BrukerkontekstMedOmråde): List<Saksbehandler> {
        ...
        return oppgaveKo.saksbehandlere.mapNotNull { saksbehandlerEpost: String ->
            saksbehandlerRepository.finnSaksbehandlerMedEpost(saksbehandlerEpost, skjermet)
                ?.takeIf { brukerkontekst.område in it.områder }.also {
                if (it == null) {
    ...
    @WithSpan
    fun slett(oppgaveKoId: Long, område: Områder, skjermet: Boolean) {
        oppgaveKoRepository.slett(oppgaveKoId, område, skjermet)
        runBlocking {
