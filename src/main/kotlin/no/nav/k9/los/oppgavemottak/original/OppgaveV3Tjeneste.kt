...
class OppgaveV3Tjeneste(
    ...
    fun erNyEksternversjon(oppgaveDto: OppgaveDto, tx: TransactionalSession): Boolean {
        return !oppgaveV3Repository.finnesFraFør(tx, oppgaveDto.eksternId, oppgaveDto.eksternVersjon)
    }

    fun eksistererFraFør(eksternId: String, type: String, område: String, tx: TransactionalSession): Boolean {
        return oppgaveV3Repository.hentOppgaveIdStatusOgHøyesteInternversjon(tx, eksternId, type, område).first != null
    }

    fun lagreNyOppgaveversjon(
    ...
    fun hentOppgaveversjon(
        ...
        val områdeObj = områdeRepository.hentOmråde(område, tx)
        return oppgaveV3Repository.hentOppgaveversjon(
            område = områdeObj,
            oppgavetype = oppgavetypeRepository.hentOppgavetype(områdeObj, oppgavetype, tx),
            eksternId = eksternId,
    ...
    @VisibleForTesting
    fun hentHøyesteInternVersjon(oppgaveEksternId: String, oppgaveTypeEksternId: String, områdeEksternId: String, tx: TransactionalSession): Int? {
        val (_, _, versjon) = oppgaveV3Repository.hentOppgaveIdStatusOgHøyesteInternversjon(tx, oppgaveEksternId, oppgaveTypeEksternId, områdeEksternId)
        return versjon
    }

    private fun hentEksternIdForOppgaverMedStatus(oppgavetypeEksternId: String, områdeEksternId: String, oppgavestatus: Oppgavestatus, tx: TransactionalSession): List<String> {
        val område = områdeRepository.hentOmråde(områdeEksternId, tx)
        val oppgavetype = oppgavetypeRepository.hentOppgavetype(område, oppgavetypeEksternId, tx)
        return oppgaveV3Repository.hentEksternIdForOppgaverMedStatus(oppgavetype, område, oppgavestatus, tx)
    }

    private fun hentSisteEksternVersjon(områdeEksternId: String, oppgaveTypeEksternId: String, oppgaveEksternId: String, tx: TransactionalSession): String? {
        val område = områdeRepository.hentOmråde(områdeEksternId, tx)
        val oppgavetype = oppgavetypeRepository.hentOppgavetype(område, oppgaveTypeEksternId, tx)
