...
class InnloggetBrukerApiTest : AbstractPostgresTest() {
    ...
    fun `returnerer alle områder PDP gir basistilgang til`() {
        ...
        runBlocking {
            val id = saksbehandlerRepository.opprettSaksbehandler("saksbehandler@nav.no", Områder.K9)
            saksbehandlerRepository.leggTilOmråde(id, Områder.AKTIVITETSPENGER)
        ...
        install(Koin) {
            modules(
                module {
                    ...
                    single {
                        no.nav.k9.los.infrastruktur.brukerkontekst.BrukerkontekstFactory(
                            no.nav.k9.los.infrastruktur.abac.Gruppeoppsett(),
                            no.nav.k9.los.infrastruktur.abac.tilganger.IngenPdpTilgangsskygge,
                            lokaleTilganger = true,
                        )
                    }
