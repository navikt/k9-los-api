...
class OppgaveKoTest : AbstractK9LosIntegrationTest() {
    ...
    fun `sjekker at oppgavekø kan opprettes og slettes`() {
        ...
        oppgaveKoRepository.slett(oppgaveKo.id, Områder.K9, false)
        assertFailure {
    ...
    fun `sjekker at oppgavekø kan få saksbehandler tilknyttet og fjernet`() {
        ...
        oppgaveKoRepository.slett(oppgaveKoFraDb2.id, Områder.K9, false)
    }
    ...
    fun `oppgavekø skal ikke være tilgjengelig fra et annet område`() {
        ...
        assertFailure { oppgaveKoRepository.slett(k9Kø.id, Områder.AKTIVITETSPENGER, false) }
    ...
    private fun mockLeggTilSaksbehandler(saksbehandlerepost: String): Long {
        ...
        return runBlocking {
            saksbehandlerRepository.opprettSaksbehandler(saksbehandlerepost, Områder.K9)
        }
    }

    @Test
    fun `direkte id gir ikke tilgang til skjermet kø eller medlemskap`() {
        val repository = get<OppgaveKoRepository>()
        val saksbehandlere = get<SaksbehandlerRepository>()
        val kø = repository.leggTil("Skjermet", true, Områder.K9)
        val medlem = saksbehandlere.opprettSaksbehandler("skjermet@nav.no", Områder.K9, true)
        val medMedlem = repository.endre(kø.copy(saksbehandlerIds = listOf(medlem)), true, Områder.K9)

        assertFailure { repository.hent(kø.id, false, Områder.K9) }
        assertFailure { repository.slett(kø.id, Områder.K9, false) }
        assertFailure { repository.endre(medMedlem.copy(tittel = "Endret"), false, Områder.K9) }
        assertFailure { repository.kopier(kø.id, "Kopi", true, true, false, Områder.K9) }
        assertThat(repository.hent(kø.id, true, Områder.K9).saksbehandlerIds).isEqualTo(listOf(medlem))
    }

    @Test
    fun `medlemskap krever samme område og skjerming og rulles tilbake ved avvisning`() {
        val repository = get<OppgaveKoRepository>()
        val saksbehandlere = get<SaksbehandlerRepository>()
        val kø = repository.leggTil("Ordinær", false, Områder.K9)
        val medlem = saksbehandlere.opprettSaksbehandler("medlem@nav.no", Områder.K9)
        val medMedlem = repository.endre(kø.copy(saksbehandlerIds = listOf(medlem)), false, Områder.K9)
        val skjermet = saksbehandlere.opprettSaksbehandler("kode6@nav.no", Områder.K9, true)
        get<no.nav.k9.los.oppgavedefinisjon.omraade.OmrådeRepository>().lagre(Områder.AKTIVITETSPENGER.eksternId)
        val annetOmråde = saksbehandlere.opprettSaksbehandler("aktivitet@nav.no", Områder.AKTIVITETSPENGER)

        for (id in listOf(skjermet, annetOmråde)) {
            assertFailure { repository.endre(medMedlem.copy(saksbehandlerIds = listOf(id)), false, Områder.K9) }
            assertThat(repository.hent(kø.id, false, Områder.K9).saksbehandlerIds).isEqualTo(listOf(medlem))
        }
        assertFailure { repository.slett(kø.id, Områder.AKTIVITETSPENGER, false) }
        assertThat(repository.hent(kø.id, false, Områder.K9).saksbehandlerIds).isEqualTo(listOf(medlem))
    }

    @Test
    fun `legacy køpayload uten område er fortsatt K9`() {
        val kø = get<OppgaveKoRepository>().leggTil("Legacy", false, Områder.K9)
        val mapper = OppgaveKoRepository.objectMapper
        val json = mapper.valueToTree<com.fasterxml.jackson.databind.node.ObjectNode>(kø)
        json.remove("område")
        assertThat(mapper.treeToValue(json, no.nav.k9.los.ko.dto.OppgaveKo::class.java).område).isEqualTo(Områder.K9)
    }
}
