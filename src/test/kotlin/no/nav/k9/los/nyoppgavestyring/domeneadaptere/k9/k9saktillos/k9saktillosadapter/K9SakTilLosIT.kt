...
class K9SakTilLosIT : AbstractK9LosIntegrationTest() {
    ...
    fun `Behandling uten åpne aksjonspunkter skal ikke vises i køer`() {
        ...
        val antallIKø = oppgaveKøTjeneste.hentAntallOppgaverForKø(kø.id, skjermet, filtrerReserverte)
    ...
    fun `Behandlinger på vent skal ikke vises i køer, men skal kunne reserveres`() {
        ...
        val opprettetUtenÅpneAksjonspunkter =
            BehandlingProsessEventDtoBuilder(eksternId).venterPåInntektsmelding().build()
        eventHandler.prosesser(opprettetUtenÅpneAksjonspunkter)
    ...
    fun `Åpne behandlinger skal kunne plukkes og fjernes fra kø`() {
        ...
        val skjermet1 = runBlocking<Boolean> { pepClient.harTilgangTilKode6() }
        val filtrerReserverte1 = true
        val antallIKøEtterRes = oppgaveKøTjeneste.hentAntallOppgaverForKø(kø.id, skjermet1, filtrerReserverte1)
    ...
    private fun assertAntallIKø(kø: OppgaveKo, forventetAntall: Int) {
        ...
        val skjermet = runBlocking<Boolean> { pepClient.harTilgangTilKode6() }
        val filtrerReserverte = true
        val antallIKøEtterRes = oppgaveKøTjeneste.hentAntallOppgaverForKø(kø.id, skjermet, filtrerReserverte)
