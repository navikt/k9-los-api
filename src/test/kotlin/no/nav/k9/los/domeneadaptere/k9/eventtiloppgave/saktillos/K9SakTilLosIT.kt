...
class K9SakTilLosIT : AbstractK9LosIntegrationTest() {
    ...
    fun `Behandling uten åpne aksjonspunkter skal ikke vises i køer`() {
        ...
        val skjermet = runBlocking { with(kontekst(TestSaksbehandler.SARA.navident!!)) { pepClient.harTilgangTilKode6() } }
    ...
    fun `Åpne behandlinger skal kunne plukkes og fjernes fra kø`() {
        ...
        val skjermet1 = runBlocking { with(kontekst(TestSaksbehandler.SARA.navident!!)) { pepClient.harTilgangTilKode6() } }
    ...
    fun `Reservasjoner skal ikke annulleres ved udefinert venteårsak på manuelt aksjonspunkt`() {
        ...
        k9SakEventHandler.prosesser(
            eventBuilder.vurderSykdom().build()
        )
        ...
        val skjermet = runBlocking<Boolean> { with(kontekst(TestSaksbehandler.SARA.navident!!)) { pepClient.harTilgangTilKode6() } }
        val filtrerReserverte = true
        val antallIKø = oppgaveKøTjeneste.hentAntallOppgaverForKø(
            oppgaveKoId = kø.id,
            filtrerReserverte = filtrerReserverte,
        ...
        assertReservasjonMedAntallOppgaver(
            TestSaksbehandler.SARA,
            1
        ) //oppgave uten AP får status UAVKLART, og blir ikke reservert
    ...
    fun `Reservasjon skal annulleres hvis alle behandlinger i reservasjonen er avsluttet eller på vent`() {
        ...
        val kø = opprettKøFor(
            TestSaksbehandler.SARA,
            querySomKunInneholder(listOf(eksternId1, eksternId2), Oppgavestatus.AAPEN)
        )
    ...
    fun `Retur fra beslutter skal ikke henge igjen`() {
        ...
        val behandling1 =
            K9SakEventDtoBuilder(eksternId1, saksnummer = saksnummer, pleietrengendeAktørId = "PLEIETRENGENDE_ID")
        k9SakEventHandler.prosesser(behandling1.vurderSykdom().build())
        ...
        val behandling2 =
            K9SakEventDtoBuilder(eksternId2, saksnummer = saksnummer, pleietrengendeAktørId = "PLEIETRENGENDE_ID")
        k9SakEventHandler.prosesser(behandling2.vurderSykdom().build())
    ...
    private fun opprettKøFor(saksbehandler: Saksbehandler, oppgaveQuery: OppgaveQuery): OppgaveKo {
        ...
        val skjermet = runBlocking { with(kontekst(saksbehandler.navident!!)) { pepClient.harTilgangTilKode6() } }
    ...
    private fun kontekst(navIdent: String): Brukerkontekst {
        ...
        return TestKontekstFactory.brukerkontekst(Områder.K9, idToken)
    }
    ...
    private fun querySomKunInneholder(
        ...
        val filtre = mutableListOf(
            byggFilter(
                ...
                *eksternId.map { it.toString() }.toTypedArray()
            )
        )
        if (status.isNotEmpty()) {
            filtre.add(
